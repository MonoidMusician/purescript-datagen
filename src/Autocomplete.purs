...
customElemConf :: AC.Config (Tuple (Qualified Proper) AKindV)
customElemConf = AC.defaultConfig
  { itemText = itemText
  , itemDisplay = \(Tuple p k) -> HH.div_ [ HH.text (show p), HH.br_ , HH.code_ [HH.text ("Kind: " <> showAKind k)] ]
  ...
  where
    ...
    itemFilter input = case decompose input of
      Nothing -> const false
      Just (Tuple moduleInput typeInput) -> \(Tuple q _) ->
        let
...
main = HA.runHalogenAff do
  ...
  HA.selectElement (QuerySelector "#customElement") >>= traverse_
    (runUI (AC.component customElemConf) items)
